...
void Texture2ColorEffect::SetPVWMatrixConstant(eastl::shared_ptr<ConstantBuffer> const& pvwMatrix)
{
    mPVWMatrixConstant = pvwMatrix;
    mProgram->GetVShader()->Set("PVWMatrix", mPVWMatrixConstant);
}

void Texture2ColorEffect::SetTexture(eastl::shared_ptr<Texture2> const& texture)
{
    mTexture = texture;
#if defined(_OPENGL_)
