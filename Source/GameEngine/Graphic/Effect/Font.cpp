    ...
    : mMaxMessageLength(maxMessageLength)
{
    ...
    mVertexBuffer = eastl::make_shared<VertexBuffer>(vformat, numVertices);
    ...
    mIndexBuffer = eastl::make_shared<IndexBuffer>(IP_TRIMESH, numTriangles,
        sizeof(unsigned int));
    ...
    mTexture = eastl::make_shared<Texture2>(DF_R8_UNORM, width, height);
