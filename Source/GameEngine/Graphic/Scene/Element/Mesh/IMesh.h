...
class IMesh : public std::enable_shared_from_this<IMesh>
{
public:
	...
	mesh buffer. */
	virtual shared_ptr<IMeshBuffer> GetMeshBuffer(u32 nr) const = 0;

	//! Get pointer to a mesh buffer which fits a material
	/** \param material: material to search for
	\return Pointer to the mesh buffer or 0 if there is no such
	mesh buffer. */
	virtual shared_ptr<IMeshBuffer> GetMeshBuffer( const Material &material) const = 0;
