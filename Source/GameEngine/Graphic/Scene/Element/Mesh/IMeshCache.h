...
#ifndef _IMESHCACHE_H_INCLUDED_
#define _IMESHCACHE_H_INCLUDED_
...
public:
	...
	this name. */
	virtual void AddMesh(const path& name, const shared_ptr<IAnimatedMesh>& mesh) = 0;
	...
	virtual void RemoveMesh(const shared_ptr<IMesh>& mesh) = 0;
	...
	virtual int GetMeshIndex(const shared_ptr<IMesh>& mesh) const = 0;
	...
	number. */
	virtual shared_ptr<IAnimatedMesh> GetMeshByIndex(u32 index) = 0;

	//! Returns a mesh based on its name.
	/** \param name Name of the mesh. Usually a filename.
	\return Pointer to the mesh or 0 if there is none with this number. */
	virtual shared_ptr<IAnimatedMesh> GetMeshByName(const path& name) = 0;

	//! Get the name of a loaded mesh, based on its index.
	/** \param index: Index of the mesh, number between 0 and getMeshCount()-1.
	\return The name if mesh was found and has a name, else the path is empty. */
	virtual const NamedPath& GetMeshName(u32 index) const = 0;

	//! Get the name of the loaded mesh if there is any.
	/** \param mesh Pointer to mesh to query.
	\return The name if mesh was found and has a name, else the path is empty. */
	virtual const NamedPath& GetMeshName(const shared_ptr<IMesh>& mesh) const = 0;
	...
	virtual bool RenameMesh(u32 index, const path& name) = 0;
	...
	\param name New name for the mesh.
	\return True if mesh was renamed. */
	virtual bool RenameMesh(const shared_ptr<IMesh>& mesh, const path& name) = 0;
