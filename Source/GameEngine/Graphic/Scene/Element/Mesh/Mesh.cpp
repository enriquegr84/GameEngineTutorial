...
#include "Graphics/Direct3D9/D3D9Renderer.h"
...
unsigned int SdkMeshResourceLoader::GetLoadedResourceSize(void *rawBuffer, unsigned int rawSize)
{
...
//
bool SdkMeshResourceLoader::LoadResource(void *rawBuffer, unsigned int rawSize, const shared_ptr<ResHandle>& handle)
{
	E_RENDERER_TYPE renderer = g_pGameApp->GetRendererImpl();

	if (renderer == GameEngineApp::Renderer_D3D9)
	{
		GE_ASSERT(0 && "This is not supported in D3D9");
	}
	else if (renderer == GameEngineApp::Renderer_D3D11)
	{
	...
	GE_ASSERT(0 && "Unsupported Renderer in SdkMeshResourceLoader::VLoadResource");
	return false;
}
*/
...
HRESULT D3DMeshNode9::VOnRestore(Scene *pScene)
{
    ...
    unsigned long *rgdwAdjacency = NULL;
    ...
    rgdwAdjacency = new unsigned long[m_pMesh->GetNumFaces() * 3];
...
D3DShaderMeshNode9::D3DShaderMeshNode9(const ActorId actorId, 
	WeakBaseRenderComponentPtr renderComponent,
	ID3DXMesh *Mesh, 
	stringc fxFileName,			// used to be effect pointer - changed to fix a VOnRestore crash.
	RenderPass renderPass, 
	const matrix4 *t)
...
HRESULT D3DShaderMeshNode9::VOnRestore(Scene *pScene)
{
	...
	unsigned long dwShaderFlags = D3DXFX_NOT_CLONEABLE | D3DXSHADER_DEBUG | D3DXSHADER_NO_PRESHADER;
	HRESULT hr;
...
HRESULT D3DShaderMeshNode9::VRender(Scene *pScene)
{
	...
	matrix4 worldViewProj = pScene->GetActiveCamera()->GetWorldViewProjection(pScene);
	matrix4 world = pScene->GetTopMatrix();
...
//
D3DTeapotMeshNode9::D3DTeapotMeshNode9(const ActorId actorId, WeakBaseRenderComponentPtr renderComponent, stringc fxFileName, RenderPass renderPass, const matrix4 *t)
...
HRESULT D3DTeapotMeshNode9::VOnRestore(Scene *pScene)
{
	...
	matrix4 rotateY90;
	rotateY90.BuildRotationY(-GE_PI / 2.0f);
	IDirectVertexBufferBuffer9* pVB = NULL;
	m_pMesh->GetVertexBuffer(&pVB);
	Vector3<float>* pVertices = NULL;
...
D3DShaderMeshNode11::D3DShaderMeshNode11(const ActorId actorId, 
	...
	RenderPass renderPass, 
	const matrix4 *t)
: SceneNode(actorId, renderComponent, renderPass, ESNT_MESH, t)
{
...
float D3DShaderMeshNode11::CalcBoundingSphere(CDXUTSDKMesh *mesh11)
{
    ...
    {
		Vector3<float> extents = mesh11->GetMeshBBoxExtents(subset);
