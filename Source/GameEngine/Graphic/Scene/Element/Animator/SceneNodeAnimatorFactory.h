...
#ifndef _SCENE_NODE_ANIMATOR_FACTORY_H_INCLUDED_
#define _SCENE_NODE_ANIMATOR_FACTORY_H_INCLUDED_

#include "GameEngineStd.h"

#include "Scenes/SceneNodes.h"
#include "Platforms/ICursorControl.h"
...
public:
	...
	SceneNodeAnimatorFactory(const shared_ptr<ICursorControl>& cursor);

	virtual ~SceneNodeAnimatorFactory();
	...
	virtual SceneNodeAnimator* CreateSceneNodeAnimator(E_SCENE_NODE_ANIMATOR_TYPE type, Scene* pScene);
	...
	\return Returns pointer to the new scene node animator or null if not successful. You need to
	drop this pointer after calling this, see IReferenceCounted::drop() for details. */
	virtual SceneNodeAnimator* CreateSceneNodeAnimator(const char* typeName, Scene* pScene);

	//! returns amount of scene node animator types this factory is able to create
	virtual unsigned int GetCreatableSceneNodeAnimatorTypeCount() const;
	...
	virtual E_SCENE_NODE_ANIMATOR_TYPE GetCreateableSceneNodeAnimatorType(unsigned int idx) const;

	//! returns type name of a createable scene node animator type
	/** \param idx: Index of scene node animator type in this factory. Must be a value between 0 and
	getCreatableSceneNodeAnimatorTypeCount() */
	virtual const c8* GetCreateableSceneNodeAnimatorTypeName(unsigned int idx) const;
	...
	virtual const c8* GetCreateableSceneNodeAnimatorTypeName(E_SCENE_NODE_ANIMATOR_TYPE type) const;

private:

	E_SCENE_NODE_ANIMATOR_TYPE GetTypeFromName(const c8* name) const;

	shared_ptr<ICursorControl> m_CursorControl;
};
