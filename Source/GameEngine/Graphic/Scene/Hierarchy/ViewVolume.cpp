...
{
    // NOTE:  SetFrame calls OnFrameChange and SetFrustum calls OnFrustumChange,
    // and both On*Change functions call UpdatePVMatrix().  OnFrameChange,
    // OnFrustumChange, and UpdatePVMatrix are virtual, so the calls in this
    // constructor resolve to those of ViewVolume.  Derived classes that
    // need additional semantics in their constructors must apply those
    // explicitly.
    ...
    Vector4<float> const& uVector, Vector4<float> const& rVector)
{
    ...
    {
#if defined(GE_VALIDATE_COORDINATE_FRAME_ONCE)
...
{
    float halfAngleRadians = 0.5f * upFovDegrees * (float)GE_C_DEG_TO_RAD;
...
bool ViewVolume::GetFrustum(float& upFovDegrees, float& aspectRatio, float& dMin, float& dMax) const
{
        ...
        && mFrustum[VF_UMIN] == -mFrustum[VF_UMAX])
    {
        ...
        upFovDegrees = 2.0f * atan(tmp) * (float)GE_C_RAD_TO_DEG;
