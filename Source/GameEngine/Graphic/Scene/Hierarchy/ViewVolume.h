...
#define VALIDATE_COORDINATE_FRAME_ONCE

class GRAPHIC_ITEM ViewVolume
{
public:
    // Construction and destruction.  The depth range for DirectX is [0,1] and
    // for OpenGL is [-1,1].  For DirectX, set isDepthRangeZeroToOne to true.
    // For OpenGL, set isDepthRangeZeroToOne to false.
    virtual ~ViewVolume();
    ...
    //     |  0  0  0         1 |
    //     +-                  -+
    // when the GE_USE_MAT_VEC convention is used and the view matrix
    // multiplies vectors on its right, viewMat * vector4.  The transpose
    // of the displayed matrix is used for the GE_USE_VEC_MAT convention
    // and the view matrix multiplies vectors on its left, vector4 * viewMat.
    //
    // The inverse view matrix for GE_USE_MAT_VEC convention is
    //          +-            -+
    //          | r0 u0 d0 p0 |
    // V^{-1} = | r1 u1 d1 p1 |
    //          | r2 u2 d2 p1 |
    ...
    // GE_USE_MAT_VEC convention, if Y = (y0,y1,y2,1), then Y = V*X, where V is
    // the view matrix.
    ...
    // shown for GE_USE_MAT_VEC.  The transpose of these matrices are
    // used for GE_USE_VEC_MAT.
    //
...
protected:
    ...
    // on whether you define GE_USE_MAT_VEC or GE_USE_VEC_MAT.  The other
    ...
    // GE_VALIDATE_COORDINATE_FRAME_ONCE).  It allows you to trap incorrectly
...
inline bool ViewVolume::IsDepthRangeZeroOne() const
{
