...
enum GRAPHIC_ITEM DFChannelType
{
    DF_UNSUPPORTED,
    DF_BYTE,
    DF_UBYTE,
...
public:
    ...
    inline static eastl::string const& GetName(DFType type);
...
private:
    // Texel information.
    static eastl::string const msName[DF_NUM_FORMATS];
...
inline eastl::string const& DataFormat::GetName(DFType type)
{
    return msName[type];
}
