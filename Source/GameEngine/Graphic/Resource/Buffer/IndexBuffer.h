...
/*
	Index Buffers. Indices identify specific vertices within your vertex buffer
	and are employed to reduce duplication of reused vertices. A topology must
	be defined in order to interpret primitive types such as point, line or
	triangle
*/
class GRAPHIC_ITEM IndexBuffer : public Buffer
{
public:
    ...
    // If you want to trap mismatches, enable GE_VERIFY_PRIMITIVE_TYPE.
    ...
    // speed, you can use
    //   eastl::shared_ptr<IndexBuffer> ibuffer = std::make_shared<IndexBuffer>(...);
...
inline bool IndexBuffer::ValidPrimitiveType(IPType type) const
{
    ...
    {
#if defined(GE_VERIFY_PRIMITIVE_TYPE)
