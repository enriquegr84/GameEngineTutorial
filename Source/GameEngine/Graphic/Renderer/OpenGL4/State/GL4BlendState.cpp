...
#include "GL4BlendState.h"

GL4BlendState::GL4BlendState(BlendState const* blendState)
    :
    GL4DrawingState(blendState)
{
    mEnableAlphaToCoverage = blendState->mEnableAlphaToCoverage;
    mEnableIndependentBlend = blendState->mEnableIndependentBlend;
    ...
    mBlendColor = blendState->mBlendColor;
    mSampleMask = blendState->mSampleMask;
}

eastl::shared_ptr<GraphicObject> GL4BlendState::Create(void*, GraphicObject const* object)
{
    if (object->GetType() == GE_BLEND_STATE)
    {
        return eastl::make_shared<GL4BlendState>(
