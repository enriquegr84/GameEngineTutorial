...
eastl::shared_ptr<GraphicObject> DX11Texture2::Create(void* device, GraphicObject const* object)
{
    if (object->GetType() == GE_TEXTURE2)
    {
        return eastl::make_shared<DX11Texture2>(
...
ID3D11Texture2D* DX11Texture2::CreateSharedDXObject(ID3D11Device* device) const
{
	...
	if (FAILED(hr))
	{
		LogError("QueryInterface failed, hr = " + GetErrorMessage(hr));
	}
	...
	if (FAILED(hr))
	{
		LogError("GetSharedHandle failed, hr = " + GetErrorMessage(hr));
	}
	...
	if (FAILED(hr))
	{
		LogError("OpenSharedResource failed, hr = " + GetErrorMessage(hr));
	}
    return dxShared;
}
