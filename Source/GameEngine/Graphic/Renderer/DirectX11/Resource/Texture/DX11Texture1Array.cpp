...
eastl::shared_ptr<GraphicObject> DX11Texture1Array::Create(void* device, GraphicObject const* object)
{
    if (object->GetType() == GE_TEXTURE1_ARRAY)
    {
        return eastl::make_shared<DX11Texture1Array>(reinterpret_cast<ID3D11Device*>(device),
