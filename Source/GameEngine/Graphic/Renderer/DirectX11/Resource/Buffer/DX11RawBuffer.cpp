    ...
    mUAView(nullptr)
{
	...
	if (FAILED(hr))
	{
		LogError("Failed to create byte-address buffer, hr = " + GetErrorMessage(hr));
	}
    mDXObject = buffer;
...
eastl::shared_ptr<GraphicObject> DX11RawBuffer::Create(
	void* device, GraphicObject const* object)
{
    if (object->GetType() == GE_RAW_BUFFER)
    {
        return eastl::make_shared<DX11RawBuffer>(
...
void DX11RawBuffer::CreateSRView(ID3D11Device* device)
{
	...
	if (FAILED(hr))
	{
		LogError("Failed to shader resource view, hr = " + GetErrorMessage(hr));
	}
}
...
void DX11RawBuffer::CreateUAView(ID3D11Device* device)
{
	...
	if (FAILED(hr))
	{
		LogError("Failed to unordered access view, hr = " + GetErrorMessage(hr));
	}
}
