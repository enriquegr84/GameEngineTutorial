...
bool DX11Buffer::Update(ID3D11DeviceContext* context)
{
    ...
    if (numActiveBytes > 0)
    {
		...
		if (FAILED(hr)) 
		{
			LogError("Failed to map buffer, hr = " + GetErrorMessage(hr));
		}
...
bool DX11Buffer::CopyCpuToGpu(ID3D11DeviceContext* context)
{
    ...
    if (numActiveBytes > 0)
    {
		...
		if (FAILED(hr))
		{
			LogError("Failed to map staging buffer, hr = " + GetErrorMessage(hr));
		}
...
void DX11Buffer::CreateStaging(ID3D11Device* device, D3D11_BUFFER_DESC const& bf)
{
	...
	if (FAILED(hr))
	{
		LogError("Failed to create staging buffer, hr = " + GetErrorMessage(hr));
	}
}
