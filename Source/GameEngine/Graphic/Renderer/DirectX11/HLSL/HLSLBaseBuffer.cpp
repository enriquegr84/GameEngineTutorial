...
HLSLBaseBuffer::HLSLBaseBuffer(D3D_SHADER_INPUT_BIND_DESC const& desc,
    unsigned int index, unsigned int numBytes, eastl::vector<Member> const& members)
    :
...
eastl::vector<HLSLBaseBuffer::Member> const& HLSLBaseBuffer::GetMembers() const
{
    return mMembers;
}
...
void HLSLBaseBuffer::GenerateLayout(eastl::vector<MemberLayout>& layout) const
{
...
void HLSLBaseBuffer::GenerateLayout(HLSLShaderType const& parent,
    unsigned int parentOffset, eastl::string const& parentName,
