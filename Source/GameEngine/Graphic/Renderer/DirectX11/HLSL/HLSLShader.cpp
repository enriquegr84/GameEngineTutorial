...
{
	mDesc.creator = eastl::string(desc.Creator);
...
void HLSLShader::SetName(eastl::string const& name)
{
	mName = name;
}

void HLSLShader::SetEntry(eastl::string const& entry)
{
	mEntry = entry;
}

void HLSLShader::SetTarget(eastl::string const& target)
{
	mTarget = target;
}
...
eastl::string const& HLSLShader::GetName() const
{
	return mName;
}

eastl::string const& HLSLShader::GetEntry() const
{
	return mEntry;
}

eastl::string const& HLSLShader::GetTarget() const
{
	return mTarget;
}
...
eastl::vector<HLSLParameter> const& HLSLShader::GetInputs() const
{
	return mInputs;
}

eastl::vector<HLSLParameter> const& HLSLShader::GetOutputs() const
{
	return mOutputs;
}

eastl::vector<HLSLConstantBuffer> const& HLSLShader::GetCBuffers() const
{
	return mCBuffers;
}

eastl::vector<HLSLTextureBuffer> const& HLSLShader::GetTBuffers() const
{
	return mTBuffers;
}

eastl::vector<HLSLStructuredBuffer> const& HLSLShader::GetSBuffers() const
{
	return mSBuffers;
}

eastl::vector<HLSLByteAddressBuffer> const& HLSLShader::GetRBuffers() const
{
	return mRBuffers;
}

eastl::vector<HLSLTexture> const& HLSLShader::GetTextures() const
{
	return mTextures;
}

eastl::vector<HLSLTextureArray> const& HLSLShader::GetTextureArrays() const
{
	return mTextureArrays;
}

eastl::vector<HLSLSamplerState> const& HLSLShader::GetSamplerStates() const
{
	return mSamplerStates;
}

eastl::vector<HLSLResourceBindInfo> const& HLSLShader::GetResourceBindInfos() const
{
	return mRBInfos;
}

eastl::vector<unsigned char> const& HLSLShader::GetCompiledCode() const
{
...
{
	output << mName.c_str() << std::endl;
	output << mEntry.c_str() << std::endl;
	output << mTarget.c_str() << std::endl;
	output << std::endl;

	output << "Description:" << std::endl;
	output << "creator = " << mDesc.creator.c_str() << std::endl;
	int i = std::min((int)(mDesc.shaderType), 6);
	output << "shader type = " << msShaderType[i].c_str() << std::endl;
	...
	for (i = 0; i < 20; ++i)
	{
		if (mDesc.flags & (1 << i))
		{
			output << "    " << msCompileFlags[i].c_str() << std::endl;
		}
	}
	...
	output << "input primitive = "
		<< msPrimitive[mDesc.gs.inputPrimitive].c_str() << std::endl;
	output << "output topology = "
		<< msPrimitiveTopology[mDesc.gs.outputTopology].c_str() << std::endl;
	...
	output << "input primitive = "
		<< msPrimitive[mDesc.ts.inputPrimitive].c_str() << std::endl;
	output << "output primitive = "
		<< msOutputPrimitive[mDesc.ts.outputPrimitive].c_str() << std::endl;
	output << "partitioning = "
		<< msPartitioning[mDesc.ts.partitioning].c_str() << std::endl;
	output << "domain = "
		<< msDomain[mDesc.ts.domain].c_str() << std::endl;
	output << std::endl;
...
eastl::string const HLSLShader::msShaderType[] =
{
	"pixel",
	"vertex",
	"geometry",
...
eastl::string const HLSLShader::msCompileFlags[] =
{
	"D3DCOMPILE_DEBUG",
...
eastl::string const HLSLShader::msPrimitive[] =
{
...
eastl::string const HLSLShader::msPrimitiveTopology[] =
{
...
eastl::string const HLSLShader::msOutputPrimitive[] =
{
...
eastl::string const HLSLShader::msPartitioning[] =
{
...
eastl::string const HLSLShader::msDomain[] =
{
