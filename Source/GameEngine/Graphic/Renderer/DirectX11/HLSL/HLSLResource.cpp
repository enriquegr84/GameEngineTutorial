...
HLSLResource::HLSLResource(D3D_SHADER_INPUT_BIND_DESC const& desc, unsigned int numBytes)
    ...
    mNumBytes(numBytes)
{
    mDesc.name = eastl::string(desc.Name);
...
HLSLResource::HLSLResource(D3D_SHADER_INPUT_BIND_DESC const& desc,
    ...
    mNumBytes(numBytes)
{
    mDesc.name = eastl::string(desc.Name) + "[" + eastl::to_string(index) + "]";
...
eastl::string const& HLSLResource::GetName() const
{
    return mDesc.name;
}
...
{
    output << "name = " << mDesc.name.c_str() << std::endl;
    output << "shader input type = " << msSIType[mDesc.type].c_str() << std::endl;
    output << "bind point = " << mDesc.bindPoint << std::endl;
    output << "bind count = " << mDesc.bindCount << std::endl;
    output << "flags = " << mDesc.flags << std::endl;
    output << "return type = " << msReturnType[mDesc.returnType].c_str() << std::endl;
    output << "dimension = " << msSRVDimension[mDesc.dimension].c_str() << std::endl;
...
eastl::string const HLSLResource::msSIType[]
{
    "D3D_SIT_CBUFFER",
    "D3D_SIT_TBUFFER",
    "D3D_SIT_TEXTURE",
...
eastl::string const HLSLResource::msReturnType[] =
{
...
eastl::string const HLSLResource::msSRVDimension[] =
{
