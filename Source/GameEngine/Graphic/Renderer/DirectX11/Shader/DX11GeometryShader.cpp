    ...
    DX11Shader(shader)
{
	...
	if (FAILED(hr))
	{
		LogError("Failed to create geometry shader, hr = " + GetErrorMessage(hr));
	}

    mDXObject = dxShader;
}

eastl::shared_ptr<GraphicObject> DX11GeometryShader::Create(
	void* device, GraphicObject const* object)
{
    if (object->GetType() == GE_GEOMETRY_SHADER)
    {
        return eastl::make_shared<DX11GeometryShader>(
