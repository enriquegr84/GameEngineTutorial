    ...
    DX11Shader(shader)
{
	...
	if (FAILED(hr))
	{
		LogError("Failed to create vertex shader, hr = " + GetErrorMessage(hr));
	}

    mDXObject = dxShader;
}

eastl::shared_ptr<GraphicObject> DX11VertexShader::Create(
	void* device, GraphicObject const* object)
{
    if (object->GetType() == GE_VERTEX_SHADER)
    {
        return eastl::make_shared<DX11VertexShader>(
