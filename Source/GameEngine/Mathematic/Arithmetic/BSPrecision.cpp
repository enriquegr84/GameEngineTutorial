    ...
    mForBSNumber(forBSNumber)
{
    ...
    if (isFloat)
    {
        mNumBits = eastl::numeric_limits<float>::digits;  // 24
        mMinBiasedExponent =
            eastl::numeric_limits<float>::min_exponent - mNumBits;  // -149
        mMaxExponent = eastl::numeric_limits<float>::max_exponent - 1;  // 127
    }
    else
    {
        mNumBits = eastl::numeric_limits<double>::digits;  // 53
        mMinBiasedExponent =
            eastl::numeric_limits<double>::min_exponent - mNumBits;  // -1074
        mMaxExponent = eastl::numeric_limits<double>::max_exponent - 1;  // 1023
    }
}
...
BSPrecision BSPrecision::operator+(BSPrecision const& precision)
{
    ...
    {
        int32_t minBiasedExponent =
            eastl::min(mMinBiasedExponent, precision.mMinBiasedExponent);
        int32_t maxExponent =
            eastl::max(mMaxExponent, precision.mMaxExponent) + 1;
