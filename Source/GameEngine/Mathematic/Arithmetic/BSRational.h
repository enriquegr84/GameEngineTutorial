...
// GE_BINARY_SCIENTIFIC_SHOW_DOUBLE conditional define.
...
#if defined(GE_BINARY_SCIENTIFIC_SHOW_DOUBLE)
public:
...
{
    mNumerator = eastl::move(r.mNumerator);
    mDenominator = eastl::move(r.mDenominator);
#if defined(GE_BINARY_SCIENTIFIC_SHOW_DOUBLE)
    mValue = (double)*this;
#endif
    return *this;
...
RealType BSRational<UIntegerType>::Convert() const
{
    ...
    int const imax = eastl::numeric_limits<RealType>::digits - 1;
    UIntType w = 0;
...
{
    template <typename UIntegerType> 
	inline BSRational<UIntegerType> abs(BSRational<UIntegerType> const& number)
    {
