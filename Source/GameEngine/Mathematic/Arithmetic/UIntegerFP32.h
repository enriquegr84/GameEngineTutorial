...
#ifndef UINTEGERFP32_H
#define UINTEGERFP32_H

#include "Core/CoreStd.h"

#include "UIntegerALU32.h"
...
//
//#define GE_ASSERT_ON_UINTEGERFP32_OUT_OF_RANGE
...
// 'eastl::atomic<int32_t> gte::UIntegerFP32<N>::msMaxSize;'
//#define GE_COLLECT_UINTEGERFP32_STATISTICS

#if defined(GE_COLLECT_UINTEGERFP32_STATISTICS)
#include "Core/AtomicMinMax.h"
#endif
...
public:
    ...
    // Support for eastl::move.  The interface is required by BSNumber, but the
    // eastl::move of eastl::array is a copy (no pointer stealing).  Moreover,
    // a eastl::array object in this class typically uses smaller than N
    ...
    inline eastl::array<uint32_t, N> const& GetBits() const;
    inline eastl::array<uint32_t, N>& GetBits();
    ...
    // eastl::ios::binary.  The return value is 'true' iff the operation
    // was successful.
...
template <int N> inline
eastl::array<uint32_t, N> const& UIntegerFP32<N>::GetBits() const
{
    return mBits;
}

template <int N> inline
eastl::array<uint32_t, N>& UIntegerFP32<N>::GetBits()
{
    return mBits;
}
