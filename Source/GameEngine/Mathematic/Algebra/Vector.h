...
public:
    ...
    Vector(eastl::array<Real, N> const& values);
	...
	// Unary operations.
	friend Vector operator+<N, Real>(Vector const& v);
	friend Vector operator-<N, Real>(Vector const& v);

	// Linear-algebraic operations.
	friend Vector operator+<N, Real>(Vector const& v0, Vector const& v1);
	friend Vector operator-<N, Real>(Vector const& v0, Vector const& v1);
	friend Vector operator*<N, Real>(Vector const& v, Real scalar);
	friend Vector operator*<N, Real>(Real scalar, Vector const& v);
	friend Vector operator/<N, Real>(Vector const& v, Real scalar);
	friend Vector& operator+=<N, Real>(Vector& v0, Vector const& v1);
	friend Vector& operator-=<N, Real>(Vector& v0, Vector const& v1);
	friend Vector& operator*=<N, Real>(Vector& v, Real scalar);
	friend Vector& operator/=<N, Real>(Vector& v, Real scalar);
	...
	friend Real Dot<N, Real>(Vector const& v0, Vector const& v1);
	friend Real Length<N, Real>(Vector const& v, bool robust);
	friend Real Normalize<N, Real>(Vector& v, bool robust);
	...
	friend Real Orthonormalize<N, Real>(int numElements, Vector* v, bool robust);
	...
	friend Vector GetOrthogonal<N, Real>(Vector const& v, bool unitLength);
	...
	// values.
	friend bool ComputeExtremes<N, Real>(int numVectors, Vector const* v, Vector& vmin, Vector& vmax);

	// Lift n-tuple v to homogeneous (n+1)-tuple (v,last).
	friend Vector<N + 1, Real> HLift<N, Real>(Vector const& v, Real last);

	// Project homogeneous n-tuple v = (u,v[n-1]) to (n-1)-tuple u.
	friend Vector<N - 1, Real> HProject<N, Real>(Vector const& v);
	...
	friend Vector<N + 1, Real> Lift<N, Real>(Vector const& v, int inject, Real value);
	...
	friend Vector<N - 1, Real> Project<N, Real>(Vector const& v, int reject);
...
template <int N, typename Real>
Vector<N, Real>::Vector(eastl::array<Real, N> const& values)
{
	int i = 0;
...
Real Length(Vector<N, Real> const& v, bool robust)
{
	if (robust)
	{
		Real maxAbsComp = fabs(v.mTuple[0]);
		for (int i = 1; i < N; ++i)
		{
...
Vector<N, Real> GetOrthogonal(Vector<N, Real> const& v, bool unitLength)
{
	Real cmax = fabs(v.mTuple[0]);
	int imax = 0;
	for (int i = 1; i < N; ++i)
	{
		Real c = fabs(v.mTuple[i]);
		if (c > cmax)
		{
			cmax = c;
			imax = i;
	...
	result.mTuple[imax] = v.mTuple[inext];
	result.mTuple[inext] = -v.mTuple[imax];
	if (unitLength)
	{
		Real sqrDistance =
			result.mTuple[imax] * result.mTuple[imax] +
			result.mTuple[inext] * result.mTuple[inext];
		Real invLength = ((Real)1) / sqrt(sqrDistance);
		result.mTuple[imax] *= invLength;
		result.mTuple[inext] *= invLength;
	}
	return result;
}
	...
	Vector<N, Real>& vmin, Vector<N, Real>& vmax)
{
	if (v && numVectors > 0)
	{
		...
		{
			Vector<N, Real> const& vec = v.mTuple[j];
			for (int i = 0; i < N; ++i)
			{
