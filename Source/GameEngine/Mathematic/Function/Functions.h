...
public:
	...
	//! returns if x equals y, taking an explicit rounding tolerance into account
	static bool Equals(Real const& x, Real const& y, Real const & tolerance = GE_ROUNDING_ERROR);
	//! returns if a equals not zero, taking rounding errors into account
	static bool IsNotZero(Real const& x, Real const & tolerance = GE_ROUNDING_ERROR);
	//! returns if a equals zero, taking rounding errors into account
	static bool IsZero(Real const& x, Real const & tolerance = GE_ROUNDING_ERROR);
...
template <typename Real>
bool Function<Real>::Equals(Real const& x, Real const& y, Real const & tolerance = GE_ROUNDING_ERROR)
{
	return (x + tolerance >= y) && (x - tolerance <= y);
}

template <typename Real>
bool Function<Real>::IsNotZero(Real const& x, Real const & tolerance = GE_ROUNDING_ERROR)
{
	return fabs(x) > tolerance;
}

template <typename Real>
bool Function<Real>::IsZero(Real const& x, Real const & tolerance = GE_ROUNDING_ERROR)
{
	return fabs(x) <= tolerance;
}
