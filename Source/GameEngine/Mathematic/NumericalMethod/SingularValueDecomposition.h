...
private:
    ...
    eastl::vector<Real> mMatrix;  // MxN elements
    ...
    eastl::vector<Real> mDiagonal;  // N elements
    eastl::vector<Real> mSuperdiagonal;  // N-1 elements
    ...
    // for now we just use eastl::vector<Rotation> whose initial capacity is
    // K*(N-1).
    ...
    eastl::vector<GivensRotation> mRGivens;
    eastl::vector<GivensRotation> mLGivens;

    // The diagonal matrix that is used to convert S-entries to nonnegative.
    eastl::vector<Real> mFixupDiagonal;  // N elements
    ...
    eastl::vector<int> mPermutation;  // N elements
    mutable eastl::vector<int> mVisited;  // N elements

    // Temporary storage to compute Householder reflections and to support
    // sorting of columns of the orthogonal matrices.
    eastl::vector<Real> mTwoInvUTU;  // N elements
    eastl::vector<Real> mTwoInvVTV;  // N-2 elements
    mutable eastl::vector<Real> mUVector;  // M elements
    mutable eastl::vector<Real> mVVector;  // N elements
    mutable eastl::vector<Real> mWVector;  // max(M,N) elements
};
    ...
    int sortType)
{
    if (mNumRows > 0)
    {
        ...
        eastl::copy(input, input + numElements, mMatrix.begin());
        Bidiagonalize();
        ...
        // are large.
        Real maxAbsComp = eastl::abs(input[0]);
        for (int i = 1; i < numElements; ++i)
        {
            Real absComp = eastl::abs(input[i]);
        ...
        Real const epsilon = eastl::numeric_limits<Real>::epsilon();
        ...
        for (unsigned int j = 0; j < mMaxIterations; ++j)
        {
            ...
            for (int i = mNumCols - 2; i >= 0; --i)
            {
                ...
                Real sum = eastl::abs(a00) + eastl::abs(a11);
                if (sum + eastl::abs(a01) != sum)
                {
                    if (imax == -1)
                    {
                        imax = i;
...
void SingularValueDecomposition<Real>::GetU(Real* uMatrix) const
{
    ...
    eastl::fill(uMatrix, uMatrix + mNumRows*mNumRows, (Real)0);
    ...
    {
        // Sorting was requested.
        eastl::fill(mVisited.begin(), mVisited.end(), 0);
...
void SingularValueDecomposition<Real>::GetV(Real* vMatrix) const
{
    ...
    // Start with the identity matrix.
    eastl::fill(vMatrix, vMatrix + mNumCols*mNumCols, (Real)0);
    ...
    int imax, Real threshold)
{
    ...
    {
        if (eastl::abs(mDiagonal[i]) <= threshold)
        {
...
void SingularValueDecomposition<Real>::ComputePermutation(int sortType)
{
    ...
    eastl::vector<SortItem> items(mNumCols);
    int i;
...
{
    // No default initialization for fast creation of eastl::vector of objects
    // of this type.
}
