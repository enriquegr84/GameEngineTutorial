    ...
    int numCols, Real* Y) const
{
    ...
    eastl::vector<Real> localInverseM;
    if (!wantInverse)
    {
    ...
    eastl::vector<int> colIndex(numRows), rowIndex(numRows), pivoted(numRows);
	eastl::fill(pivoted.begin(), pivoted.end(), 0);
    ...
    for (int i0 = 0; i0 < numRows; ++i0)
    {
        ...
        for (i1 = 0; i1 < numRows; ++i1)
        {
            if (!pivoted[i1])
            {
                for (i2 = 0; i2 < numRows; ++i2)
                {
                    if (!pivoted[i2])
                    {
                        Real absValue = fabs(matInvM(i1, i2));
        ...
        if (row != col)
        {
            ...
            {
                eastl::swap(matInvM(row, i), matInvM(col, i));
            }

            if (B)
            {
				eastl::swap(X[row], X[col]);
            }
            ...
            if (C)
            {
                ...
                {
					eastl::swap(matY(row, i), matY(col, i));
                }
            }
        }
    ...
    if (wantInverse)
    {
        ...
        for (i1 = numRows - 1; i1 >= 0; --i1)
        {
            if (rowIndex[i1] != colIndex[i1])
            {
                for (i2 = 0; i2 < numRows; ++i2)
                {
					eastl::swap(matInvM(i2, rowIndex[i1]),
...
void GaussianElimination<Real>::Set(int numElements, Real const* source, Real* target) const
{
    ...
    {
        // The inputs are not eastl containers, so ensure assignment works
        // correctly.
        if (source)
        {
