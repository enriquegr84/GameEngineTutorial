...
#ifndef BASEFILELIST_H
#define BASEFILELIST_H
...
public:
	...
	virtual unsigned int GetFileCount() const = 0;
	...
	virtual const eastl::wstring& GetFileName(unsigned int index) const = 0;

	//! Gets the full name of a file in the list including the path, based on an index.
	/** \param index is the zero based index of the file which name should
	be returned. The index must be less than the amount getFileCount() returns.
	\return File name of the file. Returns 0 if an error occured. */
	virtual const eastl::wstring& GetFullFileName(unsigned int index) const = 0;
	...
	virtual unsigned int GetFileSize(unsigned int index) const = 0;

	//! Returns the file offset of a file in the file list, based on an index.
	/** \param index is the zero based index of the file which should be returned.
	The index must be less than the amount getFileCount() returns.
	\return The offset of the file in bytes. */
	virtual unsigned int GetFileOffset(unsigned int index) const = 0;
	...
	virtual unsigned int GetID(unsigned int index) const = 0;
	...
	virtual bool IsDirectory(unsigned int index) const = 0;
	...
	virtual int FindFile(const eastl::wstring& filename, bool isFolder=false) const = 0;

	//! Returns the base path of the file list
	virtual const eastl::wstring& GetPath() const = 0;
	...
	virtual unsigned int AddItem(const eastl::wstring& fullPath, 
		unsigned int offset, unsigned int size, bool isDirectory, unsigned int id=0) = 0;
