...
CameraRig::CameraRig(eastl::shared_ptr<Camera> const& camera, float translationSpeed, float rotationSpeed)
{
    Set(camera, translationSpeed, rotationSpeed);
}

void CameraRig::Set(eastl::shared_ptr<Camera> const& camera, float translationSpeed, float rotationSpeed)
{
    mCamera = camera;
...
void CameraRig::ClearMotions()
{
    ...
    mIndirectMap.clear();
    mNumActiveMotions = 0;
    eastl::fill(mActiveMotions.begin(), mActiveMotions.end(), nullptr);
}
...
void CameraRig::Register(int trigger, MoveFunction function)
{
    if (trigger >= 0)
    {
        ...
        {
            mIndirectMap.insert(eastl::make_pair(trigger, function));
        }
    }
    else
    {
