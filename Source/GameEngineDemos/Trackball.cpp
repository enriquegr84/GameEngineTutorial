...
{
    mRoot = eastl::make_shared<Node>();
    mInitialOrientation.MakeIdentity();
}

Trackball::Trackball(int xSize, int ySize, eastl::shared_ptr<Camera> const& camera)
    :
    mXSize(0),
    mYSize(0),
    mMultiplier(0.0f),
...
void Trackball::Set(int xSize, int ySize, eastl::shared_ptr<Camera> const& camera)
{
...
void Trackball::Attach(eastl::shared_ptr<Spatial> const& object)
{
...
void Trackball::Detach(eastl::shared_ptr<Spatial> const& object)
{
    if (mValidTrackball && object)
    {
...
void Trackball::UpdateOrientation()
{
    ...
    {
        angle = acos(eastl::min(eastl::max(dot, -1.0f), 1.0f));
    }
