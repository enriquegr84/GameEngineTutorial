...
const char* PlasmaFire::Name = "PlasmaFire";

//---------------------------------------------------------------------------------------------------------------------
// PlasmaFire
//---------------------------------------------------------------------------------------------------------------------
PlasmaFire::PlasmaFire(void)
{
	mExplosionTime = 0.f;
}

bool PlasmaFire::Init(tinyxml2::XMLElement* pData)
{
...
tinyxml2::XMLElement* PlasmaFire::GenerateXml(void)
{
...
void PlasmaFire::Apply(eastl::weak_ptr<Actor> pActor)
{
	eastl::shared_ptr<Actor> pStrongActor(pActor);
	if (pStrongActor)
	{
		LogInformation("Applying plasma firing to actor id " + eastl::to_string(pStrongActor->GetId()));
	}
}

void PlasmaFire::Update(float deltaMs)
{
	// drop timing counter
	if (mExplosionTime)
	{
		if (deltaMs >= mExplosionTime)
		{
			...
			if (pTransformComponent)
			{
				...
				GameLogic::Get()->CreateActor("actors/quake/effects/plasmaexplosion.xml", nullptr, &initTransform);
